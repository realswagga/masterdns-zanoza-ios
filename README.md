...
В менеджере резолверов доступна кнопка **Scan ISP / regional DNS**. Она принимает
список DNS из DHCP/роутера или список оператора, может добавить только
ограниченное соседство `/24` вокруг приватных seed-адресов, показывает ход и
журнал, сохраняет найденный пул как родительский пресет и передаёт его в
обычный MTU/latency/throughput evaluator. Отдельное действие выбора создаёт
дочерний пресет; автоматического сканирования всего `10/8` нет.

Тест пропускной способности использует ограниченное окно передачи: если за
это время получены байты, частичный ответ сохраняется, в конце считаются объём,
скорость и ping туннеля. Тайм-аутом считается только попытка без единого байта;
это не отбрасывает резолверы, которые продолжают поток после закрытия окна.
